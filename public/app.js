(() => {
  ...
  const LEGACY_STORAGE_KEY = "umkm-accounting-cycle-v1";
  ...
  function legacyState() {
    try {
      ...
      if (saved && Array.isArray(saved.transactions) && Array.isArray(saved.adjustments)) return saved;
    } catch (_) {}
    return null;
  }
  ...
  function setSyncStatus(status, detail = "") {
    ...
    const labels = { loading: "Memuat data cloud", saving: "Menyimpan perubahan", saved: "Tersimpan di cloud", error: "Sinkronisasi gagal" };
  ...
  async function loadCloudState() {
    setSyncStatus("loading", "Menghubungkan akun");
    try {
      const response = await fetch("/api/state", { cache: "no-store" });
      if (!response.ok) throw new Error("load_failed");
      const payload = await response.json();
      cloudUserEmail = payload.user?.email || "";
      if (payload.state) {
        state = withDefaults(payload.state);
      } else {
        state = withDefaults(legacyState() || sampleState());
      ...
      state = withDefaults(legacyState() || sampleState());
      syncShell();
      setSyncStatus("error", "Data contoh ditampilkan sementara");
      render();
    }
  }
  ...
  async function saveState(silent = false) {
    ...
    setSyncStatus("saving", "Jangan tutup halaman");
    try {
      const response = await fetch("/api/state", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ state }) });
      if (!response.ok) throw new Error("save_failed");
      if (sequence === saveSequence) setSyncStatus("saved");
    } catch (_) {
      setSyncStatus("error", "Perubahan belum tersimpan");
      if (!silent) showToast("Data belum tersimpan. Periksa koneksi lalu coba lagi.", "error");
    }
  }
  ...
  async function downloadExport(format, payload) {
    ...
    try {
      const response = await fetch(`/api/export/${format}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      ...
      showToast(`${format.toUpperCase()} berhasil dibuat.`);
    } catch (_) {
      showToast(`Export ${format.toUpperCase()} gagal. Coba lagi.`, "error");
    } finally {
  ...
  document.getElementById("resetButton").addEventListener("click", () => {
    if (!window.confirm("Muat ulang data contoh? Semua data cloud pada akun ini akan diganti.")) return;
